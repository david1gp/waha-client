import type { MediaVoiceConvertOptions } from "./mediaVoiceConvertOptions.js"
...
const mediaVoiceConvertOptionsSchema = a.object({
  config: configSchema,
...
export async function mediaVoiceConvert(options: MediaVoiceConvertOptions): PromiseResult<Uint8Array> {
  const op = "mediaVoiceConvert"
  const parsed = a.safeParse(mediaVoiceConvertOptionsSchema, options)
  ...
  return wahaRequest({
    config,
    method: "POST",
    path: wahaPathSession(sessionR.data, "/media/convert/voice"),
