...
export async function sessionStop(options: SessionPathOptions): PromiseResult<SessionDTO> {
  const op = "sessionStop"
  ...
  return wahaRequest<SessionDTO>({
    config,
    method: "POST",
    path: wahaPathApi(`/sessions/${encodeURIComponent(session)}/stop`),
  })
}
