...
export function groupInfoResponseNormalize(raw: unknown, op: string): Result<GroupInfo> {
  ...
  if (jid === undefined) return createResultError(op, "Group response jid is required")

  const name = value.name ?? value.subject ?? value.Name
  if (name === undefined) return createResultError(op, "Group response name is required")
