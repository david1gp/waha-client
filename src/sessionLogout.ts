...
export async function sessionLogout(options: SessionPathOptions): PromiseResult<SessionDTO> {
  const op = "sessionLogout"
  ...
  return wahaRequest<SessionDTO>({
    config,
    method: "POST",
    path: wahaPathApi(`/sessions/${encodeURIComponent(session)}/logout`),
  })
}
